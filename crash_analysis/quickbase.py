...
Source: https://github.com/oysterhotels/quickbase/blob/master/quickbase.py
...
class Client(object):
    ...
    def _build_request(cls, **request_fields):
        ...
        b'<qdbapi><a>1</a><d f="1">e</d><b>c</b></qdbapi>'
        >>> Client._build_request(f=['a', 'b'])
        b'<qdbapi><f>a</f><f>b</f></qdbapi>'
        >>> Client._build_request(f=[({'n': 1}, 't1'), ({'n': 2}, 't2')])
        b'<qdbapi><f n="1">t1</f><f n="2">t2</f></qdbapi>'
    ...
    def __init__(self, username=None, password=None, base_url='https://www.quickbase.com',
                 app_token=None, timeout=30, authenticate=True, database=None):
    ...
    def make_url(self, database):
        return self.base_url + 'db/' + database
